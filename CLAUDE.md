...
- `kylos-qpadl/` is a separate real project (`github.com/kylosarc/kylos-qpadl`, post-quantum signatures in Rust) checked out inside this working tree. It's tracked as a git submodule (see `.gitmodules`) - don't expect its contents to show up in this repo's own commits.
