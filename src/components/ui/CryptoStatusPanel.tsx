...
const LAYER_META: Record<string, LayerMeta> = {
  ...
  ml_dsa_65: {
    ...
    desc: 'Primary standard (FIPS 204)',
    color: '#00FF41',
    nistLevel: 'NIST Level 3',
  },
  sphincs_256f: {
    ...
    desc: 'Conservative anchor (FIPS 205)',
    color: '#F43F5E',
    nistLevel: 'NIST Level 5',
  },
};
...
export function CryptoStatusPanel() {
  ...
  return (
    <div
      className="backdrop-blur-2xl border p-2 md:p-4 rounded-lg font-mono text-xs"
      style={{ backgroundColor: 'rgba(10, 10, 10, 0.6)', borderColor: 'rgba(255, 255, 255, 0.1)', color: '#e5e2e1' }}
    >
      <div className="flex items-center justify-between mb-3 border-b pb-2" style={{ borderColor: 'rgba(255, 255, 255, 0.1)' }}>
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11px] md:text-xs font-bold tracking-wider" style={{ color: '#FFB300' }}>
            QPADL POST-QUANTUM CRYPTO
          </span>
        </div>
        <div className="flex items-center gap-1.5">
          <span
            className="text-[8px] md:text-[9px] px-1.5 py-0.5 rounded font-bold uppercase"
            style={{
              color: '#00FF41',
          ...
          return (
            ...
            >
              <div className="flex items-center gap-2 min-w-0">
                <span
                  ...
                  style={{
                    ...
                    boxShadow: layer.enabled ? '0 0 6px rgba(0, 255, 65, 0.6)' : 'none',
                  }}
                />
                <div className="truncate">
                  <div className="flex items-center gap-1.5">
                    <span className="font-bold text-[10px] md:text-[11px]" style={{ color: meta.color }}>
                      {meta.label}
                    </span>
                    <span className="text-[8px] opacity-60 hidden sm:inline" style={{ color: '#c4c7c8' }}>
                      ({meta.family})
                    </span>
                  </div>
                  <div className="text-[8px] opacity-50 truncate" style={{ color: '#c4c7c8' }}>
                    {meta.desc}
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 shrink-0 ml-2">
                <span className="text-[8px] px-1 py-0.5 rounded" style={{ backgroundColor: 'rgba(255, 255, 255, 0.05)', color: '#c4c7c8' }}>
                  {meta.nistLevel}
                </span>
                <span
      ...
      <div className="pt-2 border-t space-y-2" style={{ borderColor: 'rgba(255, 255, 255, 0.08)' }}>
        <button
          ...
          className="w-full py-1.5 px-3 rounded font-mono text-[9px] md:text-[10px] font-bold tracking-wider transition-all flex items-center justify-center gap-2 cursor-pointer"
          style={{
